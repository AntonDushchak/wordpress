...
class APIClientV2 {
    ...
    /**
     * Отправка HTTP запроса с улучшенной обработкой ошибок
     */
    ...
    /**
     * Отправка шаблона в API
     */
    public function send_template($template_data, $template_id, $is_update = false) {
        if ($is_update) {
            // Для обновления используем PUT с template_id в URL
    ...
    /**
     * Проверка существования шаблона на внешнем сайте
     */
    ...
    /**
     * Получение API ID пользователя (используем WordPress ID напрямую)
     */
    private function get_user_api_id($wp_user_id = null) {
        ...
        // Возвращаем WordPress ID как строку (API ожидает строку)
        return (string) $wp_user_id;
    }

    /**
     * Построение payload для создания шаблона
     */
    private function build_template_create_payload($template_data, $template_id) {
        ...
        // Получаем API ID текущего пользователя
    ...
    /**
     * Построение payload для обновления шаблона
     */
    ...
    /**
     * Нормализация немецких символов
     */
    ...
    public function sync_users($users_data) {
        ...
        foreach ($users_data as $user_data) {
            try {
                // Регистрируем пользователя через новый API endpoint
                $result = $this->register_user($user_data);

                if ($result['success']) {
                    $success_count++;
                    // API ID теперь совпадает с WordPress ID
    ...
    /**
     * Регистрация пользователя через новый API endpoint
     */
    ...
    /**
     * Маппинг ролей WordPress в роли системы
     */
    ...
    /**
     * Маппинг ролей WordPress на роли API
     */
