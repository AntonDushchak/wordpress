...
class Templates {
        ...
        function initTemplatesPage() {
            ...
            setTimeout(function() {
                ...
                if (window.NeoTemplatesCore && window.NeoTemplatesCore.init) {
                    ...
                    // Принудительно вызываем проверку синхронизации при загрузке
                    setTimeout(function() {
                    ...
                    // Также убеждаемся, что loadTemplates доступен глобально
                    window.reloadTemplatesList = function() {
                        ...
                        $.ajax({
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        ...
                                        if (response.success) {
                                            ...
                                            // Добавляем информацию о синхронизации к каждому шаблону
                            ...
                            error: function(xhr, status, error) {
                                ...
                                // Продолжаем загрузку шаблонов даже если проверка синхронизации не удалась
                                $.ajax({
                                    url: neoJobBoardAjax.ajaxurl,
                                    type: 'POST',
                                    data: {
                    ...
                    // Fallback - загружаем шаблоны вручную с проверкой синхронизации
                    loadTemplatesWithSyncCheck();
                }
                
                // Функция для загрузки шаблонов с проверкой синхронизации (fallback)
                function loadTemplatesWithSyncCheck() {
                    // Сначала проверяем синхронизацию
                    $.ajax({
                        url: neoJobBoardAjax.ajaxurl,
                        type: 'POST',
                        data: {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.success) {
                                        ...
                                        // Отображаем шаблоны с информацией о синхронизации
                ...
                // Функция для отображения шаблонов с информацией о синхронизации
                        ...
                        <form id="template-form" onsubmit="return false;">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="template-fields">
                                    <!-- Felder werden hier dynamisch hinzugefügt через NeoTemplatesFields -->
                                </div>
    ...
    public static function delete_template($template_id) {
        ...
        // Проверяем, используется ли шаблон в заявках
        ...
        // Если есть заявки, удаляем их каскадно
        if ($usage_count > 0) {
