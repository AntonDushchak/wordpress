    ...
    public static function render_page() {
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <div class="mb-3">
                                            ...
                                            <div class="form-text">Separate file types by commas</div>
                                        </div>
                                    </div>
                                </div>
        ...
        function initSettingsPage() {
            ...
            window.testApiConnection = function() {
                ...
                if (!apiUrl) {
                    showMessage('error', 'Enter API URL');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showMessage('error', response.data || 'API testing error');
                        }
                    },
            ...
            window.syncAllTemplates = function() {
                if (!confirm('Are you sure you want to synchronize all templates with API?')) {
                    return;
                }
                
                const $btn = $('[onclick="syncAllTemplates()"]');
                const originalText = $btn.html();
                $btn.prop('disabled', true).html('<i class="spinner-border spinner-border-sm me-1"></i> Synchronization...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showMessage('error', response.data || 'Synchronization error');
                        }
                    },
                    error: function(xhr, status, error) {
                        showMessage('error', 'Connection error: ' + error);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            showMessage('error', 'User synchronization error with API');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('API sync error:', error);
                        showMessage('error', 'Connection error with API');
                    }
                });
            };
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            const data = response.data;
                            let message = data.message || 'Administrators registration error';
                            ...
                            if (data.results) {
                                console.log('Registration errors:', data.results);
                            }
                        }
                    },
                    error: function(xhr, status, error) {
            ...
            window.syncUsersToApi = function() {
                if (!confirm('Are you sure you want to synchronize selected users with API?')) {
                    return;
                }
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (data.error_count > 0 && data.results) {
                                console.log('Synchronization details:', data.results);
                            }
                            
                            setTimeout(() => location.reload(), 2000);
                        } else {
                            const data = response.data;
                            let message = data.message || 'Users synchronization error';
                            ...
                            if (data.results) {
                                console.log('Synchronization errors:', data.results);
                            }
                        }
                    },
                    error: function(xhr, status, error) {
