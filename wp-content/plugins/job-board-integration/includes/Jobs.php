    ...
    public static function render_page() {
        ...
        function initJobsPage() {
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <table class="table table-sm">
                                <tr><td><strong>Application ID:</strong></td><td>#${data.application.id}</td></tr>
                                <tr><td><strong>Name:</strong></td><td>${data.application.first_name}</td></tr>
                                <tr><td><strong>Last name:</strong></td><td>${data.application.last_name}</td></tr>
                                <tr><td><strong>Responsible:</strong></td><td>
                                    <span id="responsible-display">${data.application.responsible_employee || 'Not assigned'}</span>
                                    <button class="btn btn-sm btn-outline-secondary ms-2" onclick="editResponsible(${data.application.id})" title="Edit">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                </td></tr>
                                <tr><td><strong>Status:</strong></td><td><span class="badge bg-primary">${data.application.status}</span></td></tr>
                                <tr><td><strong>Submission date:</strong></td><td>${new Date(data.application.created_at).toLocaleString('ru-RU')}</td></tr>
                            </table>
                        </div>
                        <div class="col-md-6">
                            <h6>Template</h6>
                            <table class="table table-sm">
                                <tr><td><strong>Name:</strong></td><td>${data.template ? data.template.name : 'Deleted template'}</td></tr>
                                <tr><td><strong>Description:</strong></td><td>${data.template ? (data.template.description || '-') : '-'}</td></tr>
                            </table>
                        </div>
                    </div>
                    <div class="mt-4">
                        <div class="alert alert-info">
                            ...
                            <strong>Note:</strong> Detailed application data has been sent to the API and is not stored in WordPress. 
                            For full information, please contact the administrator.
                        </div>
                    </div>
                `;
            ...
            window.editResponsible = function(applicationId) {
                ...
                const newResponsible = prompt('Enter the name of the responsible employee:', currentResponsible === 'Not assigned' ? '' : currentResponsible);
                ...
                if (newResponsible !== null) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                $('#responsible-display').text(newResponsible || 'Not assigned');
                                alert('Responsible employee updated');
                            } else {
                                alert('Error updating: ' + (response.data || 'Unknown error'));
                            }
                        },
                        error: function(xhr, status, error) {
                            alert('Verbindungsfehler: ' + error);
